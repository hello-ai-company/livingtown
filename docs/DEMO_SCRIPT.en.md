...
1. Open the MAP phase and show the three current tools: `contribute_knowledge`, `verify_knowledge`, and `query_area`. In Simple mode, explain that these appear as friendly actions rather than technical names. In Advanced mode, show the `Auto`, `Japan (GSI)`, and `Worldwide (OpenFreeMap)` basemap choices and move overseas to demonstrate the worldwide provider.
2. In Simple mode, show **Around You Now**, then enter a rainy-crosswalk sentence in the visible one-line composer. Press Enter to open the preview, review the derived category/time/safe public summary/location treatment, and click **Post** explicitly. Advanced mode still provides the five-step correction flow: location → category → condition → confidence → description/review/privacy. Keep the description under 200 characters and confirm that it contains no personal information. Show the new `PENDING` visual and its detail card.
...
1. Open MAP in Simple/English and show **Around You Now**, then enter “A bicycle was reportedly stolen near here yesterday.” in the visible “What's happening here?” composer. Press Enter, review the theft/incident metadata, yesterday-relative observation time, safe public summary, and coarse-location warning, then click **Post**. Confirm the Community report wording, coarsened neutral marker, and no route change.
