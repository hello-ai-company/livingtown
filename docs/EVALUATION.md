...
- 未検証、agree 1票、agree 2票、disagreeでthreshold未満、wheelchair限定barrier、同一入力の決定性、`avoided.reason` と実際の `edge_ids` の対応を `src/sim/route.test.ts` で確認。
...
- `verifier_id` は `anon-...` 形式に限定し、氏名・メールのような値を拒否。
- demo fixtureの既存カウンタと匿名verificationを対応させた。
...
- **「PIIを保持しない」コード境界はPASS。** これは「共有環境で完全に匿名」を意味しない。

### Testable fallback and seed

- WebMCPオブジェクトが存在しない通常Node/Vitest環境でも、同じtool definitionとSIMULATED surfaceを検証できる。
- `npm run seed` がグラフ、knowledge、匿名verification、household fixtureを生成する。
...
通常ブラウザのSIMULATED fallbackでは、投稿 → 匿名2主体による追認 → 経路変更 → replayを確認できる。これは実機WebMCPのPASS証跡ではない。

### Supabase security design

`supabase/migrations/0002_verification_privacy_rls.sql` にverification unique制約、household scope／expiry／label制約、RLS、匿名roleのwrite禁止を追加した。migrationのSQL設計は確認済みだが、共有Supabase projectへの適用・認証済みwrite adapter・監査ログは未確認である。
...
| `npm run seed` | PASS — 6 nodes / 7 edges / 10 knowledge / 13 anonymous votes / 3 households |
