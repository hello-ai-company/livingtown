...
Phase 6のPR #4 run `33310283020` / job `99253976986` とPR #6の最新run `33361713392` / job `99394196838` も同じ状態（`conclusion=failure`、`runner_id=0`、`runner_name=""`、`steps=[]`、`gh run view --log-failed` は `log not found`）である。checkout／Node／npmのstep開始証跡がないため、今回もコードfailureとは判定せず `CI_INFRA_BLOCKED` と記録する。
