...
- MAP now exposes exactly `contribute_knowledge`, `verify_knowledge`, and `query_area`. The Phase 8 ownership migration and Phase 10 observation migration are applied as an Expand rollout; the disposable database gate passes 169 pgTAP tests, and the Phase 10.3 shared identity／CRUD／privacy／Realtime gate is PASS. The current-head Native WebMCP gate is recorded separately from the historical Phase 7 evidence.
...
- 既存テストを維持し、trust-boundary／Realtime／GeoJSON projection／i18n／CRUD／worldwide basemap／geolocation／living observation／first-use UX testsを追加した。現在は23 files / 149 tests。sensitive public-summary、分類漏れfallback、relative time、future timestamp、category-change metadata、preview、My Reports、native clusteringも実装テストで確認する。
