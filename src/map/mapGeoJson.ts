...
export function createKnowledgeFeatureCollection(
  ...
  return featureCollection(views.map((view) => ({
    ...
    properties: {
      ...
      routeImpact: view.routeImpact,
      trustState: view.trustState,
      report_type: view.item.report_type ?? 'persistent_condition',
      source_kind: view.item.source_kind ?? 'community',
      location_precision_m: view.item.location_precision_m ?? 0,
