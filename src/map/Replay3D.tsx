...
export function Replay3D({ snapshot, locale, mode, onView3D }: Replay3DProps) {
  ...
  return (
    <section className="replay3d-placeholder">
      ...
      <div className="replay3d-placeholder__actions"><span>{Object.keys(snapshot.routes).length} {t('replay.routes')}</span><button type="button" className="secondary-button" onClick={onView3D}>{t('replay.view3d')} <span>↗</span></button></div>
    </section>
  )
}
